...
(* Definition of the erdos cnf *)
Definition lerdos60 := snd (gen_all 1161 60).

Definition erdos60 := l2a lerdos60.

(* This is the computationnal part of the proof *)
Lemma erdos60th : 
   forall rho, ~ valid (interp_var rho)  erdos60.
Proof.
 apply (@checker_correct erdos60 ("erdos1161_60.reso"%string)).
 native_cast_no_check (refl_equal true). (* The computation is done during the Qed. *) 
Time Qed.

Lemma eval_erdos env : 
    eval_cnf env lerdos60 -> valid (interp_var (n2i env)) erdos60.
Proof.
move=> H.
