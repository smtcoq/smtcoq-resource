...
Proof. intros; subst; auto using to_list_In. Qed.
...
Lemma afold_right_implb_false : forall A a f,
  ...
  afold_right bool A false implb f a = false.
Proof.
  intros A a f; intros [H2 H3]; apply afold_right_ind; auto; intros b i H4 H5; rewrite H5, H2; auto.
Qed.
...
Proof.
  ...
  rewrite eqb_spec in Heq1. intros _. left. auto.
  intro H; right.
  ...
  pose (P j k := k = false -> (forall i : int, (j <= i) = true -> (i < length a - 1) = true -> f (a .[ i]) = true) /\ f (a .[ length a - 1]) = false); assert (H: P 0 (afold_right bool A false implb f a)).
  generalize (afold_right_Ind _ _ P false implb f a); rewrite Heq2; intro IH; apply IH; clear IH; unfold P.
...
Lemma afold_right_implb_true_aux : forall A a f,
  (0 < length a = true /\ exists i, i < length a - 1 = true /\ f (a.[i]) = false) ->
  afold_right bool A false implb f a = true.
Proof.
  intros A a f; case_eq (length a == 0); intro Heq1.
  rewrite eqb_spec in Heq1. rewrite Heq1. intros [H _]. discriminate.
  case_eq (length a <= 1); intro Heq2.
  intros [_ [i [Hi _]]]; elim (ltb_0 i); replace 0 with (length a - 1); auto; rewrite eqb_false_spec in Heq1; apply to_Z_inj; rewrite to_Z_sub_1_diff; auto; assert (H1: [|length a|] <> 0%Z) by (intro H; apply Heq1, to_Z_inj; auto); rewrite leb_spec in Heq2; generalize (leb_0 (length a)); rewrite leb_spec; change [|0|] with 0%Z; change [|1|] with 1%Z in Heq2; omega.
  intros [_ H]. revert H.
  pose (P j k := (exists i : int, (j <= i) = true /\ (i < length a - 1) = true /\ f (a .[ i]) = false) -> k = true); assert (H: P 0 (afold_right bool A false implb f a)).
  generalize (afold_right_Ind _ _ P false implb f a); rewrite Heq2; intro IH; apply IH; clear IH; unfold P.
...
Proof.
  ...
  intro H. rewrite eqb_spec in H. rewrite H. intros [H1 _]. discriminate.
  intros Heq [H10 [H1|H1]].
...
Proof.
  ...
  intros H; split.
    rewrite eqb_false_spec in Heq1. rewrite ltb_spec. assert (H1: [|length a|] <> [|0|]) by (intro H1; apply Heq1, to_Z_inj; auto). assert (H2:=leb_0 (length a)). rewrite leb_spec in H2. omega.
  revert H. case_eq (length a <= 1); intro Heq2.
  unfold afold_right; rewrite Heq1, Heq2; intro H; right; intros i Hi; replace i with 0; auto; apply to_Z_inj; rewrite ltb_spec in Hi; rewrite eqb_false_spec in Heq1; assert (H1: [|length a|] <> 0%Z) by (intro H1; apply Heq1, to_Z_inj; auto); rewrite leb_spec in Heq2; change [|1|] with 1%Z in Heq2; generalize (leb_0 (length a)) (leb_0 i); rewrite !leb_spec; change [|0|] with 0%Z;  omega.
  pose (P j k := k = true -> (exists i : int, (j <= i) = true /\ (i < length a - 1) = true /\ f (a .[ i]) = false) \/ (forall i : int, (j <= i) = true -> (i < length a) = true -> f (a .[ i]) = true)); assert (H: P 0 (afold_right bool A false implb f a)).
  generalize (afold_right_Ind _ _ P false implb f a); rewrite Heq2; intro IH; apply IH; clear IH; unfold P.
  intros b i H1 H2 H3; case_eq b; intro Heq3.
  ...
  unfold P in H; intro H1; destruct (H H1) as [[i [_ H2]]|H2].
  left; exists i; auto.
