...
Module S.
  ...
  Definition rget (r:resource) (s:t) (pos:int) := 
    get s (Resource.geti r pos).

  Definition set_resolve (r:resource) (spos:t * int) : t * int :=
    let (s,pos) := spos in
    let len := Resource.geti r pos in
    if len == 0 then (s,pos)
    else
      let pos := pos + 4 in
      let set := Resource.geti r pos in
      let pos := pos + 4 in
      let (c,pos) := 
        fold (fun (cp:C.t * int) => let (c,pos) := cp in (C.resolve (rget r s pos) c, pos+4))
         2 len (rget r s pos, pos+4) in
      (internal_set s set c, pos).
  ...
  Proof.
    ...
    assert (Hsurj : (forall A B (p:A * B), p = (fst p, snd p))) by (intros A B (p1,p2);trivial). 
     rewrite (Hsurj _ _ (fold f 2 (geti r pos) (rget r s (pos + 4 + 4), pos + 4 + 4 + 4))).
    apply valid_internal_set;trivial.
    apply fold_ind. unfold rget;simpl;auto.
    intros (c,p) Hc;unfold f;simpl. apply C.resolve_correct;auto;apply Hv.
  Qed.
