  ...
  match Form.pform l with
    | Fatom ha ->
      (match Atom.atom ha with
        ...
        | _ -> failwith "SmtTrace.get_eq: equality was expected")
    | _ -> failwith "SmtTrace.get_eq: equality was expected"
  ...
  match Form.pform l with
    | Fatom ha ->
      (match Atom.atom ha with
        ...
        | _ -> false)
    | _ -> failwith "SmtTrace.get_eq: atom was expected"
...
let mkTrans p =
  ...
  match concl with
    |[c] ->
      ...
      Other (EqTr (c,cert))
    |_ -> failwith "SmtTrace.mkTrans: no conclusion or more than one conclusion in transitivity"
...
let rec process_congr a_args b_args prem res =
  match a_args,b_args with
    ...
    | [],[] -> List.rev res
    | _ -> failwith "SmtTrace.process_congr: incorrect number of arguments in function application"
      ...
      (match Atom.atom a, Atom.atom b with
        ...
        | Aapp (a_f,a_args), Aapp (b_f,b_args) ->
          if a_f = b_f then
            ...
            Other (EqCgr (c,cert))
          else failwith "SmtTrace.mkCongr: left function is different from right fucntion"
        | _, _ -> failwith "SmtTrace.mkCongr: atoms are not applications")
    |_ -> failwith "SmtTrace.mkCongr: no conclusion or more than one conclusion in congruence"
          ...
          (match Atom.atom (get_at c), Atom.atom (get_at p_p) with
            ...
            | Aapp (a_f,a_args), Aapp (b_f,b_args) ->
              if a_f = b_f then
                let cert = process_congr (Array.to_list a_args) (Array.to_list b_args) prem_val [] in
                Other (EqCgrP (p_p,c,cert))
              else failwith "SmtTrace.mkCongrPred: unmatching predicates"
            | _ -> failwith "SmtTrace.mkCongrPred : not pred app")
        |_ ->  failwith "SmtTrace.mkCongr: no or more than one predicate app premice in congruence")
    |[] ->  failwith "SmtTrace.mkCongrPred: no conclusion in congruence"
    |_ -> failwith "SmtTrace.mkCongrPred: more than one conclusion in congruence"
