...
Require Import List PArray Bool Int31 ZMicromega.
...
Section Operators.
  ...
  Section AUX.
    ...
    (*   rewrite Int31Properties.eqb_spec in H1; subst l1; auto. *)
    ...
    Proof.
      ...
      (* Atom *)
      unfold is_true; rewrite Int31Properties.eqb_spec; intro; subst a; auto.
      (* Interesting case *)
      apply interp_check_distinct; auto.
      (* Double negation *)
      unfold is_true; rewrite andb_true_iff, Int31Properties.eqb_spec; intros [H1 H2]; subst j1. rewrite (interp_check_lit _ _ H2). auto.
      (* Conjunction *)
    ...
    (*   apply interp_check_distinct; auto. *)
    (*   (* Double negation *) *)
    (*   unfold is_true; rewrite andb_true_iff, Int31Properties.eqb_spec; intros [H1 H2]; subst j1; apply (fold_ind2 _ _ (fun x y => x = true -> y = true)). *)
    (*   apply interp_check_lit; auto. *)
    (*   intros a b; case a; try discriminate; intros H _; rewrite H; auto. *)
    (*   (* Conjunction *) *)
    (*   unfold is_true; rewrite andb_true_iff, Int31Properties.eqb_spec; intros [H1 H2]; rewrite forallbi_spec in H2; intro H3; assert (H4 := afold_left_andb_true_inv _ _ _ H3); clear H3; apply afold_left_andb_true; rewrite <- H1; intros i Hi; eapply interp_check_lit; eauto. *)
    (*   (* Disjunction *) *)
    (*   unfold is_true; rewrite andb_true_iff, Int31Properties.eqb_spec; intros [H1 H2]; rewrite forallbi_spec in H2; intro H3; assert (H4 := afold_left_orb_true_inv _ _ _ H3); clear H3; destruct H4 as [i [H3 H4]]; eapply afold_left_orb_true. *)
    (*   rewrite <- H1; eauto. *)
    (*   eapply interp_check_lit; eauto. *)
    (*   (* Implication *) *)
    (*   unfold is_true; rewrite andb_true_iff, Int31Properties.eqb_spec; intros [H1 H2]; rewrite forallbi_spec in H2; intro H3; apply afold_right_implb_true; case_eq (length a1 == 0); intro Heq. *)
  ...
  Section Valid.
    ...
    Proof.
      ...
      rewrite Int31Properties.eqb_spec in H; rewrite H; auto.
